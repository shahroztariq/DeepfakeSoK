...
class MAT(nn.Module):
    def __init__(self, net='efficientnet-b4',feature_layer='b2', attention_layer='b5', num_classes=2, M=4, mid_dims=256,\
    dropout_rate=0.25,drop_final_rate=0.5, pretrained=False,alpha=0.05,size=(380,380),margin=0.5,inner_margin=[0.1,-2]):
    ...
    def forward(self, x,y=0,train_batch=False, AG=None, return_ft=False):
        if train_batch:
            if AG is None:
        ...
        feature_matrix=torch.cat((feature_matrix,projected_final),1)
        if return_ft:
            intermediate = self.ensemble_classifier_fc[0:2](feature_matrix)
        ensemble_logit=self.ensemble_classifier_fc(feature_matrix)
        
        return ensemble_logit if not return_ft else  (ensemble_logit, intermediate)
