    ...
    for i, (frame_clip, record_clip) in enumerate(zip(clips, detect_results)):
        ...
        for j, track in enumerate(tracks):
            ...
            for (box, ldm, _), frame in zip(track, frame_clip):
                ...
                new_images.append((frame[y1:y2, x1:x2], big_box, new_box, new_ldm5, ldm68))
...
def detect_all(file, sfd_only=False, return_frames=False, max_size=None, provided_frames=None):
    # frames = grab_all_frames(file, max_size=max_size, cvt=True)
    if provided_frames is None:
        frames = grab_all_frames(file, max_size=max_size, cvt=True)
    else:
        frames = [frame[..., ::-1] for frame in provided_frames[:max_size]] # Flip to RGB
    if not sfd_only:
        detect_res = flatten([detector.detect(item) for item in partition(frames, 50)])
